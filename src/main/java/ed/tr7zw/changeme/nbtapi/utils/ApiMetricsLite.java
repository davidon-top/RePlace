package ed.tr7zw.changeme.nbtapi.utils;
						...
						if (plugin instanceof JsonObject) {
							...
							try {
								...
								if (logFailedRequests) {
									MinecraftVersion.getLogger().log(Level.WARNING, "[NBTAPI][BSTATS] Encountered exception while posting request!", e);
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					if (logFailedRequests) {
						MinecraftVersion.getLogger().log(Level.WARNING, "[NBTAPI][BSTATS] Could not submit plugin stats of " + plugin.getName(), e);
						// Not using the plugins logger since the plugin isn't the plugin containing the NBT-Api most of the time
